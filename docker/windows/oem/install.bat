...
REM 3) PC-Inventory-Agent als Windows-Dienst installieren
REM Hinweis: agent.exe muss im OEM-Ordner liegen (per "make cross" erzeugt).

set "INSTALLDIR=%ProgramFiles%\PC-Inventory"
set "DATADIR=%ProgramData%\PC-Inventory"
...
echo [PC-Inventory] DNS auf Samba-DC %DC_IP% gesetzt.
...
echo [PC-Inventory] Verwaltungs-Features installiert (GPMC, RSAT-AD-Tools, RSAT-DNS-Server).

REM --- Agent installieren (nur wenn vorhanden) ---
if not exist "%~dp0agent.exe" (
  echo [PC-Inventory] agent.exe fehlt im OEM-Ordner - Agent uebersprungen.
  goto :eof
)
...
if not exist "%DATADIR%\agent.yaml" (
  ...
  >> "%DATADIR%\agent.yaml" echo ca_cert_path: "C:/ProgramData/PC-Inventory/cert.pem"
  >> "%DATADIR%\agent.yaml" echo interval: "30s"
  >> "%DATADIR%\agent.yaml" echo state_path: "C:/ProgramData/PC-Inventory/agent-state.json"
)

"%INSTALLDIR%\agent.exe" -config "%DATADIR%\agent.yaml" install
"%INSTALLDIR%\agent.exe" -config "%DATADIR%\agent.yaml" start
echo [PC-Inventory] Agent-Dienst installiert und gestartet.
